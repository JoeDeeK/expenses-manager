...
const sessionStorage = createCookieSessionStorage({
    cookie: {
        secure: process.env.NODE_ENV === 'propduction',
        secrets: [SESSION_SECRET],
        sameSite: 'lax',
        maxAger: 30 * 24 * 60 * 60, // 30 days
        httpOnly: true
    }
});
...
export async function requireUserSession(request) {
    ...
    // do not throw a catch boundary, instead redirect user
...
export async function signup({email, password}) {
    ...
    // install bcryptjs to hash password for safe db storage
